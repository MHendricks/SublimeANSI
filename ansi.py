...
class AnsiCommand(sublime_plugin.TextCommand):
    ...
    def _colorize_ansi_codes(self, edit):
        ...
        ansi_unsupported_codes = fast_view_find_all(view, r"\x1b\[(0;)?(2?[24578]m|2[39]m)")
...
class AnsiColorBuildCommand(Default.exec.ExecCommand):
    ...
    def on_data_process(self, proc, data):
        ...
        unsupported_pattern = r"\x1b\[(0;)?(2?[24578]m|2[39]m)"
...
def generate_color_scheme(cs_file, settings):
    ...
    theme = {
        "name": "Ansi",
        "author": "Auto-generated by ANSIescape plugin",
        "globals": {camel_to_snake(k): v for k, v in settings.get("GENERAL").items()},
        "rules": [],
    }
